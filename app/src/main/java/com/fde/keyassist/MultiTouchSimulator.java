...
//        Device.injectEvent(event, 0, 2);
        ...
        while (now < endtime){
            ...
            coords[1].y += 3; // 第二个触摸点向下移动
            coords[1].x += 3; // 第一个触摸点向下移动
...
//        Device.injectEvent(event, 0, 2);
    ...
    public static void simulateSingleFingerTouch(Instrumentation instrumentation) {
        long downTime = SystemClock.uptimeMillis();
        long eventTime = SystemClock.uptimeMillis();
...
//        Device.injectEvent(event, 0, 2);
        ...
        while (now < endtime){
            ...
            eventTime = SystemClock.uptimeMillis();
            event = MotionEvent.obtain(
                    downTime, eventTime, MotionEvent.ACTION_MOVE, 1,
                    properties, coords, 0, 0, 1, 1,
                    deviceId, 0, source, 0);
